...
def receive():
    ...
    try: # try catch to handle lost connection
        ...
        if Server_message == "Too many failed attempts, ending connection": # if server sends a message about failing password close out client
            print(Server_message)
            return True # return true to end main loop
        
        elif Server_message == "Correct password, entering main menu":
            print(Server_message)
            status = receive()
            return status # return False to continue main loop
        
        elif Server_message == "close client": # if server sends a close client command
            print('closing out connection')
            return True # return true to end main loop
            
        elif 'What will you ask?' in Server_message:
            system('cls')
            print(Server_message)
            return str
        
        elif 'Please enter a number between 0 and 100' in Server_message:
            system('cls')
            print(Server_message)
            return status
        
        elif 'Please enter Rock (1), Paper (2), Or Scissors (3)' in Server_message:
            system('cls')
            print(Server_message)
            return status
        
        else: # assumming everything is fine continue main loop and print what server had to say
            print(Server_message)
            return status # return false to continue main loop
