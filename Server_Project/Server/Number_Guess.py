...
def main_server(send, receive):
    ...
    send("Please enter a number between 0 and 100, your score starts at 50 but for every guess wrong 1 is subtracted!") 
    ...
    while True:
        ...
        # loop to determine if user answer matches random number
        while True:
            user_input = receive()
            ...
            if user_input == random_number:
                if user_input == 69:
                    send(f"Congrats you got it right it was: {random_number}, Nice!\nYour score is: {score}" +
                      "\nDo you want to Play Again (1), Return to Main Menu (2), or Exit (3)?")
                    break
                else:
                    send(f"Congrats you got it right it was: {random_number}\nYour score is: {score}" +
                      "\nDo you want to Play Again (1), Return to Main Menu (2), or Exit (3)?")
                    break
        ...
        # loop to check if user wants to play again or leave
        while True:
            user_input = receive()

            match user_input:
                case 1:
                    send('Please enter a number between 0 and 100')
                    break
                case 2:
                    return
                case 3: 
